...
-- Table `FANFOU`.`ORDER`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `FANFOU`.`ORDER` (
...
USE `FANFOU`$$
CREATE DEFINER = CURRENT_USER TRIGGER `FANFOU`.`ORDER_BEFORE_INSERT` BEFORE INSERT ON `ORDER` FOR EACH ROW
BEGIN
  SET NEW.CREATED_DATETIME = NOW();
END
  $$
...
END
  ...
  DELIMITER ;
CREATE USER 'fanfou' IDENTIFIED BY 'fanfou';
